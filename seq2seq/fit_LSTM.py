...
with open('VNESEcorpus_no_accent.txt', encoding='utf-8') as f:
    train_data = f.readlines()
# you may also want to remove whitespace characters like `\n` at the end of each line
train_data = [str(x.strip()) for x in train_data]

with open('VNESEcorpus.txt', encoding='utf-8') as f:
...
# validation_input = input_texts[(len(input_texts) / 10 * 9):]
# validation_target = target_texts[(len(input_texts) / 10 * 9):]
# input_texts = input_texts[:(len(input_texts) / 10 * 9)]
# target_texts = target_texts[:(len(input_texts) / 10 * 9)]


def loop_batch(input_texts, target_texts, input_token_index, target_token_index):
...
with open('input_dict.txt', 'w') as file:
    file.write(json.dumps(input_token_index))

with open('output_dict.txt', 'w') as file:
    file.write(json.dumps(target_token_index))
